...
# 🇨 🇭 🇦 🇱 🇴      🇸 🇮 🇷    🇵 🇱 🇪 🇦 🇸 🇪    🇬 🇪 🇹 🇴 🇺 🇹    
    ...
    async def opner(event):
            if event.query.user_id == bot.uid :
                ...
                await event.edit("`>>>\n\nReopened The Main Menu of \n©𝕄𝔸ℍ𝔸𝔻𝔼𝕍 𝕌𝕊𝔼ℝ𝔹𝕆𝕋` ", buttons=dc)
            else:
                reply_pop_up_alert = "Please get your own Userbot,for more info visit @MAHADEV_TANDAV_SUPPORT!"
  ...
  #       🇮 🇹 🇳 🇦    🇰 🇾 🇺   🇸 🇵 🇾     🇰 🇷    🇷 🇭 🇪     🇭 🇴      🇸 🇭 🇦 🇦 🇩 🇮    🇰 🇷 🇳 🇮    🇭    🇰 🇾 🇦   🇧 🇸 🇩 🇰 
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            Cobra = "Please get your own Userbot, and don't use mine for more info visit @MAHADEV_TANDAV_SUPPORT!"
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:  # pylint:disable=E0602
            ...
            await event.edit(buttons=dc)
              ...
              TheDark = "Please get your own Userbot😁😁,for more info visit @MAHADEV_TANDAV_SUPPORT! 😎😎"
 ...
 #hehehehehhehhehhehe   
    ...
    async def on_plug_in_callback_query_handler(event):
        if not event.query.user_id == bot.uid:
            atul= "Please get your own Userbot😁😁,for more info visit @MAHADEV_TANDAV_SUPPORT! 😎😎"
        ...
        try:
            if event.query.user_id == bot.uid :
                ...
                reply_pop_up_alert = "Please get your own Userbot, and don't use mine for more info visit @MAHADEV_TANDAV_SUPPORT!"
...
#  🇦 🇷 🇪      🇧 🇸 🇩 🇰      🇮 🇸 🇸 🇪    🇰 🇦 🇳 🇬  🇲 🇦 🇹   🇰 🇷    🇷 🇪   🇲 🇨 
                    ...
                    try:
                        ...
                        await event.edit( "`Your 𝕄𝔸ℍ𝔸𝔻𝔼𝕍 𝕌𝕊𝔼ℝ𝔹𝕆𝕋 Has Successfully loaded` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e))+ "𝕄𝔸ℍ𝔸𝔻𝔼𝕍 𝕌𝕊𝔼ℝ𝔹𝕆𝕋 Has Successfully loaded" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
                    fcix = [custom.Button.inline("  🅱🅰🅲🅺 ",data="back({})".format(shivam)),custom.Button.inline(" 𝕮𝖑𝖔𝖘𝖊 ", data="close"),custom.Button.inline(" 🆄🅽🅻🅾🅰🅳 ",data="unload({})".format(shivam_sh1vam))]
                    reply_pop_up_alert = "Please get your own Userbot,for more info visit @MAHADEV_TANDAV_SUPPORT!"
                    await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
              if event.query.user_id == bot.uid :
                    ...
                    try:
                        ...
                        await event.edit( "`Your 𝕄𝔸ℍ𝔸𝔻𝔼𝕍 𝕌𝕊𝔼ℝ𝔹𝕆𝕋 Has Successfully unloaded` >>>" + str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
                    except Exception as e:
                        await event.edit("Error{}".format(shortname, str(e)) +"𝕄𝔸ℍ𝔸𝔻𝔼𝕍 𝕌𝕊𝔼ℝ𝔹𝕆𝕋 Has Successfully unloaded"+ str(event.data_match.group(1).decode("UTF-8")),buttons=fcix)
              else:
                    shortname = event.data_match.group(1).decode("UTF-8")
                    fcix = [custom.Button.inline("  🅱🅰🅲🅺 ",data="back({})".format(shivam)),custom.Button.inline(" 🅲🅻🅾🆂🅴 ", data="close"),custom.Button.inline(" 🅻🅾🅰🅳 ",data="load({})".format(shivam_sh1vam))]
                    reply_pop_up_alert = "Please get your own Userbot,for more info visit @MAHADEV_TANDAV_SUPPORT!"
